...
```
> the range of energy used are similar in a and b address as both from around 0 to 450 therms per month.The lower use around 50, but peak b has a peak at a lower therms values compare to address a. In contrast, the high use of therm around 350, but the peak of A is higher than the peak of B.
...
ggplot(energy, aes(y = hdd, x = renovated)) + 
  ...
  facet_wrap(~ address)
```
> the outdoor temperature is one of lurking variable as it happened to be clder outside after renovation.
...
-   After renovation...
-   the weather become colder.
-   which then leads to higher energy use.

Thus, *when controlling for* outdoor temperature
...
ggplot(starbucks, aes(y = Latitude, x = Longitude)) + 
  geom_point(size = 0.5)
```
>it is unclear to see and undersrand. There is no function to enlarge or shrink(interactive).
...
>ALthough starbucks distribute all around the world, the location of starbucks concentrate on three plots 
...
ggplot(elections_by_counties, aes(map_id = county_fips, fill = median_rent)) +
  ...
  theme(legend.position = "right") + 
  coord_equal() + 
  scale_fill_gradientn(name = "median rent", colors = c("white", "purple", "red"))
```
