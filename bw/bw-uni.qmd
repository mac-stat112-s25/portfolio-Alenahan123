...
correlation and comparsion with other variables like time, evaluation, difficulty, etc.

how many hikes are categorized as easy, moderate or difficult.

>which category is the most common compared to the others.
...
range
comparison with other varibles
...
>The code create a blank camvas. The x-axes for gragh, which rating is the scale,is showing up as three categories: difficult, easy and moderate

-   What, in general, is the first argument of the `ggplot()` function?
>create a plot from ggplot2 packages and name the database.

-   What is the purpose of writing `x = rating`?
>This specifies that the rating variable
will be plot to the x-axis.
...
>add layers or components

-   We added the bars using `geom_bar()`? Why "geom"?
>used to create bar charts. It specifies the geometry  of the data.
...
fill is the filling color(interior)
...
>rating : how challenging the hikes are, categorized to easy, moderate, and difficult.

-   **variability between categories**: Are observations evenly spread out among the categories, or are some categories more common than others?
>not evenly spread.The number of hikes in moderate are much more than the other two as the bar chart is much taller.


We must then *translate* this information into the *context* of our analysis, here hikes in the Adirondacks. Summarize below what you learned from the bar chart, in context.
>The difficulty level of hike and hikers' preferences for these levels

#### Part c {.unnumbered}

Is there anything you don't like about this barplot? For example: check out the x-axis again.

easy, moderate and difficult is not display in order, making difficulties for viewing
The categories may be too general.
...
>The density plot is smoother and provides a clearer picture of the typical elevation, variability, and distribution shape. However, the histogram gives a more granular, binned view but can be less precise depending on how the bins are defined.

### Exercise 12: Density Plots vs Histograms {.unnumbered}

The histogram and density plot both allow us to visualize the behavior of a quantitative variable: typical outcome, variability / range, shape, and outliers. What are the pros/cons of each? What do you like/not like about each?

-For histogram, it is straightforward and it is great for quick understanding, but it is sensitive to bin size, which may create false pattern. In contrast, density plot  provides a smooth view of distribution, making it easier to see patterns and compare datasets. However, It can mislead if the dataset is small or if the bandwidth is poorly chosen.
...
labs(x = "Elevation (feet)", y = "Number of hikes")
```
> the ollowing lines build upon the initial point, indicating that the plot is still developing and the idea is not yet complete.
...
```
>It improves code readability and maintainability. It makes it easier to see how different parts of an expression are combined, especially when dealing with long lines. 
