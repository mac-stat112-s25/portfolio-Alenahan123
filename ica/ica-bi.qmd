...
```{r}
#The relationship is positive as the higher the supoprt the higher it was in 2020.However, there are some countries like Texaswhere the support in 2020 was much higher than that in 2016.
```
...
```
```{r}
#there is a strong negative relationship between median rent and republic support as the higher the rent, the lower the republic support. But, the relationship between median age and republic support is slightly postive. The elder the age the higher the support.
```
...
R supports tend to be the highest in red state and the lowest in blue state from the view of median and upper quatile range, although the range in all these three state is wide.
...
```{r}
#density plot. This is a plot can clearly shows distribution, range and more details.
```

-   What is one pro of density plots relative to boxplots?
```{r}
#density plots provide a smoother and more detailed visualization of the data distribution. While boxplots summarize data using five key statistics.
```

-   What is one con of density plots relative to boxplots?
```{r}
# boxplot explicitly show medians, quartiles, and potential outliers
```
...
```{r}
#for pros, it is easier to compare the portion of winner in blue, purple and red. However, it can not clearly see the count of them.
```
...
```{r}
#A side-by-side bar plot is my favorite. Because it is easier to compare and also demonstrate the count.
```
...
ggplot(weather, aes(x=temp3pm, fill = location))+geom_density()
