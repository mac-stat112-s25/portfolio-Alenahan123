...
```
>keep countries in Hawaii and Delaware
...
#   head()
```
>add a new column for the difference between the republic and dem percent in 2020.
...
#   head()
```
> add a new column for the number of republic vote
```{r}
# What did this code do?
# elections_small |> 
...
#   head()
```
>add new column for true or false the republic win.
...
elections_small |>
 mutate(change_dem_support = dem_pct_16 - dem_pct_20) |>
 head()
```
...
elections_small |>
 mutate(dem_20_higher = dem_pct_20>dem_pct_16) |>
 head()
```
...
> it can be more efficient to get rid of some rows before arranging.
...
```
> calculate the median of percentage of republic vote in 2020 across all countries.
```{r}
# What does this do?
# elections_small |> 
#   summarize(median_repub = median(repub_pct_20))
```
> calculate and name the median as median_repub
```{r}
# What does this do?
# elections_small |> 
#   summarize(median_repub = median(repub_pct_20), total_votes = sum(total_votes_20))
```
> calculate the median of percentage of republic vote and the sum of total nmuber of vote. Then name them.
...
elections_small|>
  group_by(state_name) |>
  summarize(total_vote=sum(total_votes_20))|>
