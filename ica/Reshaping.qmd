...
>In general, the reaction time increases with the lack of sleep. Some subject have a larger impact as the rate of increase is faster for some of them.
...
ggplot(billboard,aes(y=wk2, x= wk1))+
  geom_point()+
  geom_abline(interceptt=0, slope =1)
```
...
```{r}
nov_1999 |>
  pivot_longer(col=-artist, names_to ="week",names_prefix = "wk",values_to = "ranking")
```
